...
export interface ValidationMetrics {
  ...
  countryCoverage: { [country: string]: number };
}
...
export class AddressService {
   ...
   */
  async validateAddress(address: Address, options: any = {}): Promise<ValidationResult> {
   ...
   */
  async validateBatch(addresses: Address[], options: any = {}): Promise<{ index: number; validation: ValidationResult }[]> {
    const results: { index: number; validation: ValidationResult }[] = [];
  ...
  async getHealthStatus(): Promise<HealthStatus> {
    ...
    const healthyProviders = Object.values(providers).filter((p: any) => p.status === 'healthy').length;
  ...
  private async validateWithProvider(address: Address, provider: string, options: any): Promise<ValidationResult> {
  ...
  private async validateWithGoogle(address: Address, options: any): Promise<ValidationResult> {
  ...
  private async validateWithHere(address: Address, options: any): Promise<ValidationResult> {
  ...
  private async validateWithMapbox(address: Address, options: any): Promise<ValidationResult> {
    const config = this.providerConfigs.get('mapbox');
    const addressString = this.formatAddressString(address);

    const response: AxiosResponse = await axios.get(
      `${config.baseUrl}/${encodeURIComponent(addressString)}.json`,
      {
        params: {
  ...
  private parseHereResult(result: any): Address {
  ...
  private parseMapboxResult(result: any): Address {
  ...
  private validateInput(address: Address): void {
  ...
  private sanitizeAddress(address: Address): Partial<Address> {
    return {
  ...
  private updateMetrics(success: boolean, responseTime: number, provider: string): void {
  ...
  private extractGoogleComponent(components: any[], types: string[], nameType = 'long_name'): string {
  ...
  private extractMapboxContext(context: any[], type: string): string {
  ...
  private calculateGoogleConfidence(result: any): number {
    ...
    if (locationType === 'ROOFTOP') return 0.9;
    if (locationType === 'RANGE_INTERPOLATED') return 0.8;
    return 0.7;
  }

  private parseGoogleComponents(result: any): AddressComponents {
    const components = result.address_components;
    return {
  ...
  private parseGoogleCoordinates(result: any): Coordinates {
  ...
  private parseHereComponents(result: any): AddressComponents {
    const address = result.address;
    return {
  ...
  private parseHereCoordinates(result: any): Coordinates {
  ...
  private parseMapboxComponents(result: any): AddressComponents {
    const properties = result.properties;
    const context = result.context || [];
  ...
  private parseMapboxCoordinates(result: any): Coordinates {
  ...
  private getGoogleSupportedCountries(): string[] {
    return ['US', 'CA', 'GB', 'FR', 'DE', 'IT', 'ES', 'AU', 'JP', 'BR']; // Sample list
  }

  private getHereSupportedCountries(): string[] {
    return ['US', 'CA', 'GB', 'FR', 'DE', 'IT', 'ES', 'AU', 'JP', 'BR']; // Sample list
  }

  private getMapboxSupportedCountries(): string[] {
    return ['US', 'CA', 'GB', 'FR', 'DE', 'IT', 'ES', 'AU', 'JP', 'BR']; // Sample list
  }
}