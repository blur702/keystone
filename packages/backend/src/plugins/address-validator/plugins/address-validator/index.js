...
class AddressValidatorPlugin {
    constructor(context) {
        ...
        this.addressService = new AddressService_1.AddressService(context);
    }
    /**
        ...
        try {
            if (data.address) {
                ...
                if (validation.isValid && validation.standardized) {
                    ...
                    await this.context.services.database.query('UPDATE users SET address = $1, address_validated_at = NOW() WHERE id = $2', [JSON.stringify(validation.standardized), data.userId]);
     ...
     */
    async onShippingAddressChanged(data) {
        try {
            if (data.address) {
                const validation = await this.addressService.validateAddress(data.address);
     ...
     */
    async standardizeAddress(req, res) {
        try {
...
exports.default = AddressValidatorPlugin;
